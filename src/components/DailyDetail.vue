...
import {getOneDailyAPI} from "../apis/edit.js";
...
const getDaily = async () => {
  const res = await getOneDailyAPI(route.params.id);
...
<template>
  <div class="flex-col items-center w-2/3 space-y-5">
    <div class="self-start text-blue-400 font-semibold flex items-center text-2xl">
      <svg class="w-12 h-12 pr-3" viewBox="0 0 1024 1024">
        <path
      ...
      <router-link class="font-bold pr-3 text-blue-400 hover:text-blue-300"
                   :to="`/person/${daily.userId}`">
        {{ daily.username }} 的日记 <span>[ {{new Date(daily.createTime).toLocaleDateString()}}]</span>
      </router-link>
    ...
    <div class="flex items-center justify-center space-x-5 border-y py-3 border-gray-300">
      <div v-for="image in imageList" :key="image.id" >
        <el-image style="width: 100px; height: 100px" :src="image.url" :fit="image.url" />
      </div>
    </div>
    <div class="flex items-center justify-center">
      <div class="tracking-widest w-3/4">
